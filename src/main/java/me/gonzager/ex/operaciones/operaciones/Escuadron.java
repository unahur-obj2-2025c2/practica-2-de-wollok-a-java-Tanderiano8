...
public class Escuadron {
    ...
    public void agregarDron(Dron dron) {
        if (drones.size() == limiteDeDrones) {
            throw new RuntimeException("Supera la cantidad maxima defnidad por la ciudad");
        }
        drones.add(dron);
    ...
    public Boolean puedeOperarEn(Zona zona){
        return drones.stream().anyMatch(d -> d.esAvanzado()) && capacidadOperativa() > (zona.getTamanioTotal() * 2);
    }
